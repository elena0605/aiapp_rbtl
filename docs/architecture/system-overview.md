...
- **Local Dev**: Fully dockerized with `docker-compose.yml` (production mode) or `docker-compose.dev.yml` (hot-reload). Langfuse is provided externally (separate repo or cloud). Alternative: run services directly (Uvicorn + Next.js dev server) for active debugging.
